...
public class AddStudentPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		CustomButton idLabel = new CustomButton("Student Id", (int) idField.getBounds().getX(),
				(int) idField.getBounds().getY() - (int) (Utils.scale(50) * 1.2), Utils.scale(200), Utils.scale(50),
				Utils.colours[4]);
		...
		CustomButton firstNameLabel = new CustomButton("First Name", (int) firstNameField.getBounds().getX(),
				(int) firstNameField.getBounds().getY() - (int) (Utils.scale(50) * 1.2), Utils.scale(200),
				Utils.scale(50), Utils.colours[4]);
		...
		CustomButton lastNameLabel = new CustomButton("Last Name", (int) lastNameField.getBounds().getX(),
				(int) lastNameField.getBounds().getY() - (int) (Utils.scale(50) * 1.2), Utils.scale(200),
				Utils.scale(50), Utils.colours[4]);
		...
		CustomButton gradeLabel = new CustomButton("Grade", (int) gradeField.getBounds().getX(),
				(int) gradeField.getBounds().getY() - (int) (Utils.scale(50) * 1.2), Utils.scale(200), Utils.scale(50),
				Utils.colours[4]);
		gradeLabel.setSelectable(false);
		gradeLabel.draw(g, panel);

		// Submit button
		submit = new CustomButton("Submit", maxX / 2 - Utils.scale(100), 3 * maxY / 5, Utils.scale(200),
		...
		// Write if there is an error or not
		if (attemptValidation == 2) {
			g.drawString("One of the fields may have been inputted incorrectly. Please try again.",
					maxX / 2 - errorFontMetrics
							.stringWidth("One of the fields may have been inputted incorrectly. Please try again.") / 2,
	...
	public boolean addStudent() throws exceptions.StudentAlreadyExistsException {
		...
		// Calls method from signInManager
